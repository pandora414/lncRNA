...
use FindBin qw($Bin);
use lib $Bin;
die "perl <novel SNP stat dir> <dir output> <region distribution file>" unless @ARGV == 3;
my ($dir,$outdir,$region) = @ARGV;
my %hash;
my (@samples,@term);
my $flag = 1;
my @files = glob("$dir/*snp.novel_stat");
foreach my $file(@files)
{
	my $name = (split /\//,$file)[-1];
	$name =~ s/\.snp\.novel_stat//;
	my $samp;
	$flag ++;
	open IN,"$file" or die;
	while(<IN>)
...
open OUT,">$outdir/Novel_SNP.all.stat.xls" or die;
...
close OUT2;
system ("Rscript $Bin/snp_region.plot.R --argument $region,$outdir/Novel_SNP.region");
