...
pdf(paste(output,'.edit.repeat.pdf',sep=''))
 ggplot(a, aes(x = "", y = a$Count, fill = a$Iterm)) +
  ...
  theme(plot.margin=unit(c(1,-2,-2,-2),units="cm")) + ##改变图在画布上的位置，C(上，右，下，左)
  theme(legend.title = element_blank(),legend.position='top')+ ##去掉图例的标题
  scale_fill_discrete(labels = mylabel) + ## 将原来的图例标签换成现在的myLabel
  theme(axis.text.x = element_blank()) +  ## 白色的外框即是原柱状图的X轴，把X轴的刻度文字去掉即可
  geom_text(aes(y = a$Count/2 + c(0, cumsum(a$Count)[-length(a$Count)]), label = mylabel2), size = 8)
dev.off()

png(paste(output,'.edit.repeat.png',sep=''))
 ggplot(a, aes(x = "", y = a$Count, fill = a$Iterm)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar(theta = "y") +
  labs(x = "", y = "", title = "") +
