...
main: {
    ...
    if ($method eq "GLM") {
        ...
        for (my $i = 0; $i < $#A_samples; $i++) {
            ...
            #my ($sample_a, $sample_b) =  ($sample_i, $sample_j); ###   edit by yutong
...
sub run_edgeR_sample_pair {
    ...
    print $ofh "source(\"$FindBin::Bin/R/rnaseq_plot_funcs.R.pvalue\")\n";
    print $ofh "pdf(\"$output_prefix.edgeR.DE_results.MA_n_Volcano.pdf\")\n";
    print $ofh "result_table = tTags\$table\n";
    print $ofh "plot_MA_and_Volcano(result_table\$logCPM, result_table\$logFC, result_table\$PValue)\n";
    print $ofh "dev.off()\n";
